    ...
    error: (request, status, error)->
      ...
      if(request and request.responseJSON and (errors = request.responseJSON.errors))
        ...
        if(msg)
          ...
          if($("#offcanvas-signup_user_#{method}").hasClass('invalid'))
            $("#offcanvas-signup_user_#{method}").removeClass('invalid')
            $("#offcanvas-signup_user_#{method}").removeClass('error')
            $("#offcanvas-signup_user_#{method}").next('small').remove()
  })
) .
